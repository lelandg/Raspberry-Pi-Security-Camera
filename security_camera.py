...
emailAddressFrom = 'yourEmailFromAddress'
emailAddressTo = 'yourEmailToAddress'
emailSubject = 'Motion detected on camera %s' % (thisDeviceName, )
...
sipWhiteList=['sip:yourAccount@sip.linphone.org']
...
class SecurityCamera:
  def __init__(self, username='', password='', whitelist=[], camera='', snd_capture='', snd_playback=''):
    ...
    self.lastMessageTicks = time.time() - (WAITSECONDS / 2)
    self.lastEmailTicks = time.time() - (WAITEMAILSECONDS / 2)
  ...
  def emailImage(self):
    ...
    self.smtp.sendmail(emailAddressFrom, emailAddressTo, msgRoot.as_string())
  ...
  def run(self):
		while not self.quit:
		  if detectMotion and not self.core.in_call():
